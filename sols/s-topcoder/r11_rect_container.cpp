...
Rect *newRect(int w, int h) {
	Rect *res;
	if (0) /*!freeRects.empty())*/ {
		res = freeRects.back(); freeRects.pop_back();
	} else {
		res = &rectArr[ridx++];
#if DBG
		if (ridx >= sizeof(rectArr) / sizeof(rectArr[0])) {
			cout << "OOM" << endl;
			exit(-1);
		}
#endif
...
void allocFromCell(Rect *cell, Rect *rect) {
	...
	if (!leftCol && !bottomRow) {
		...
		if (bottomRow) {
			...
			if (leftCol) {
				// add
				r = newRect(rect->width, bottomRow);
...
void solve() {
	...
	D(cout << "DEBUG greedy w = " << wu << " height " << hu << endl; dbgs = 1;)
...
#if BENCH
	freopen("rect_container.txt", "r", stdin);
#endif
